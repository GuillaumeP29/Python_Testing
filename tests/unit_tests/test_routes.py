...
def test_post_purchase_places_with_wrong_data_should_status_code_ok(client):
    response = client.post('/purchasePlaces/', data={
...
def test_post_purchase_places_with_improper_data_should_status_code_ok(client):
    response = client.post('/purchasePlaces/', data={'fake data': 'fake name'})
    assert response.status_code == 400


def test_logout_should_status_code_ok(client):
